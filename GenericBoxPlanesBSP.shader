Shader "CrossSection/GenericBoxPlanesBSP" {
	Properties{
	...
	_Glossiness("Smoothness", Range(0,1)) = 0.5
		...
		_Plane4Normal("Plane3Normal",Vector) = (0,0,-1,0)
		_Plane4Position("Plane3Position",Vector) = (0,0,0,1)
	}
		SubShader{
...
#pragma target 3.0
	...
	fixed3 _Plane4Normal;
	fixed3 _Plane4Position;
	fixed3 _Plane5Normal;
	fixed3 _Plane5Position;
	fixed3 _Plane6Normal;
	...
	bool checkVisability(fixed3 worldPos)
	{
		...
		float dotProd4 = dot(worldPos - _Plane4Position, _Plane4Normal);
		float dotProd5 = dot(worldPos - _Plane5Position, _Plane5Normal);
		float dotProd6 = dot(worldPos - _Plane6Position, _Plane6Normal);

		return (dotProd1 > 0 || dotProd2 > 0) 
			|| (dotProd3 > 0 || dotProd4 > 0)
			|| (dotProd5 > 0 || dotProd6 > 0);
